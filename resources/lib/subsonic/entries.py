...
def artist_entry(conn, item, params):
    image = conn.getCoverArtUrl(item.get("coverArt"))
    return {
        "label": _starred_label(item.get("id"), item.get("name")),
        "thumb": image,
        "fanart": image,
...
def album_entry(conn, item, params):
    image = conn.getCoverArtUrl(item.get("coverArt"))
    return {
        "label": _album_label(item, params.get("hide_artist", False)),
        "thumb": image,
        "fanart": image,
...
def track_info(item):
    """The music info dict for a track - shared by listings and the player."""
    return {"music": {
...
def track_entry(conn, item, params):
    image = conn.getCoverArtUrl(item.get("coverArt"))
    return {
        "label": _track_label(item, params.get("hide_artist")),
        "thumb": image,
        "fanart": image,
