...
from .router import HANDLE, action, url_for
...
def list_albums(params):
    ...
    if len({item.get("artist") for item in items}) <= 1:
...
def list_tracks(params):
    ...
    if "album_id" in params:
        ...
        cache.seed(item["id"] for item in items if item.get("id"))
    ...
    if menu_id == "tracks_random" and len(items) >= per_page:
        listing.append(_navigate_next(params))
...
def search(params):
    ...
    result = conn.search2(query=query)
    songs = (result.get("searchResult2") or {}).get("song") or []
    if not songs:
        return browse_indexes(params)
...
def search_album(params):
    ...
    albums = (result.get("searchResult2") or {}).get("album") or []
    if not albums:
        return browse_indexes(params)

    listing = [entries.album_entry(conn, album, params) for album in albums]
    add_directory_items(create_listing(listing, content="albums"))
...
def play_track(params):
    ...
    song = library.get_song(conn, track_id)
    item = None
    if song:
        image = conn.getCoverArtUrl(song.get("coverArt"))
        item = {
            "label": "%s - %s" % (song.get("artist", ""), song.get("title", "")),
            "thumb": image,
            "fanart": image,
            "mime": song.get("contentType"),
            "info": entries.track_info(song),
        }

    set_resolved_url(url, item)
...
def star_item(params):
    ...
    if ok:
        ...
        addon.notify(addon.L(30051))  # Could not update the starred status
